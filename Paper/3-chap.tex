...
Der Marker mit \bodyCode{ID=1} soll auf der Spitze des kleinen Fingers befestigt werden.
Gegenüberliegend davon ist auf der Daumenspitze der Marker mit der ID 3 anzubringen.
Die übrigen Marker werden auf die Spitze des Mittelfingers (\bodyCode{ID=2}) und zentriert auf das \textit{HG} (\bodyCode{ID=4}) geklebt.
...
Damit auch die vertikale Position der Hand im Bild erfasst werden kann, wird wiederum die \inlinemath{y}-Koordinate des Markerbereichs berechnet.
...
Die möglichen Befehle sind in Tabelle~\TeLi{tab:cf_cmds} dargestellt.
...
Jedes Mal, wenn alle vier Marker auf der Hand erkannt werden, wird diese Stoppuhr auf \inlinemath{t=0} zurückgesetzt.
