...
ArUco-basierte Marker zählen zu den verlässlichsten, vor allem seit \TeLi{sec:ocv}{OpenCV} ein Submodul zur Implementierung dieser Marker hinzugefügt hat \cite{IJ:fiducial}.
...
Es ist zu beachten, dass alle Marker der Bibliothek \bodyCode{DICT\_4X4\_50} auch in \bodyCode{DICT\_4X4\_100} vorhanden sind, da die größere Bibliothek alle IDs der kleineren umfasst.
...
Die Gaussche Trapezformel wird zur Berechnung der Fläche \inlinemath{A} von ebenen, unregelmässigen Polygonen verwendet.
...
Wobei \inlinemath{n} die Anzahl Eckpunkte des Polygons darstellt, sowie \inlinemath{x_i} und \inlinemath{y_i} die Koordinaten des \inlinemath{i}-ten Punktes im zweidimensionalen Raum.
...
Der Ursprung des Namens ist in \aTeLi{pic:polygon} und \aTeLi{pic:shoelace} illustriert\footnotemark.
...
\begin{minipage}[t]{0.5\textwidth}
    ...
    \end{tikzpicture}
    \captionof{figure}{Illustration Shoelace-Muster.}
        \label{pic:shoelace}
\end{minipage}
...
2A = 
\begin{aligned}[t]
    & |\cp (x_1 \cdot y_2) \cp (x_2 \cdot y_3) \cp (x_3 \cdot y_4) \cp (x_4 \cdot y_5) \cp (x_5 \cdot y_1) \\
    & \cm (x_2 \cdot y_1) \cm (x_3 \cdot y_2) \cm (x_4 \cdot y_3) \cm (x_5 \cdot y_4) \cm (x_1 \cdot y_5)|
\end{aligned}
\]
