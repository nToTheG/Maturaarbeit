...
\footnotetext{Informationen der Liste stammen von \cite{Wiki:ocv}}
...
Abgesehen von diesen klassischen Anwendungsfeldern wird OpenCV auch zunehmend in Forschungsprojekten genutzt, beispielsweise zur gesten- oder markerbasierten Steuerung von Robotiksystemen, wie sie im Rahmen dieser Arbeit zum Einsatz kommt~\cite{sse:foodDel}.
...
\label{sec:aruco}
Ein ArUco-Marker (wie in \aTeLi{pic:marker0} dargestellt) ist ein quadratischer Referenzmarker, der zur Positions- und Orientierungserkennung in Bildern beziehungsweise Kamerasystemen zum Einsatz kommt.
...
Für diese Arbeit werden aus folgenden Gründen quadratische Marker benutzt:
...
\begin{figure}[H]
    ...
    \caption{ArUco-Marker mit ID 0 \cite{chev:arucogen}}
        \label{pic:marker0}
\end{figure}
...
Der entscheidende Unterschied zwischen dieser Funktionsweise und der von ArUco-Markern ist der Informationsgehalt.
...
In der \aTeLi{tab:arucoDicts} sind drei Bibliotheken und deren Eigenschaften dargestellt.
...
Für dieses Projekt wird die Bibliothek \bodyCode{DICT\_4X4\_50} verwendet, weil sie die geometrisch unkomplizierteste Bibliothek ist, die OpenCV unterstützt.
...
Dabei handelt es sich um grundlegende mathematische Konzepte, die für die Auswertung der Bilddaten vonnöten sind.
...
Jegliche Distanzen werden in dieser Arbeit durch die Subtraktion von Vektoren berechnet.
...
\begin{itemize}
    ...
    \item \textbf{Modularität:} Es existieren über 15 optionale Erweiterungs-Decks, die den Anwendungsbereich der Crazyflie gezielt erweitern.
\end{itemize}
...
Inzwischen ist mit \techWord{Crazyflie 2.1\texttt{+}} eine neue Version der erschienen.
...
\begin{itemize}
    \item Unterstützung der Steuerung über Apple- und Android-Smartphones via BLE (Bluetooth Low Energy) sowie über Computer mit angeschlossenem \hTeLi{sec:crpa}{Crazyradio~PA}.
    ...
    \item Einem Hochpräzisions-Drucksensor.
\end{itemize}
...
Das Modul ist in acht Submodule unterteilt.
...
Dieses Frequenzband wird auch von zahlreichen anderen Geräten genutzt. Dazu gehören etwa WLAN-Router, Bluetooth-Geräte und drahtlose Eingabegeräte, die das Signal zwischen Crazyradio und Crazyflie stören können. Dieses Problem lässt sich jedoch durch das Wählen anderer Kanäle umgehen~\cite{bc:crpa_conf}.

\subsection{Funkverbindung}
Um eine Verbindung zwischen dem Crazyradio und einer Crazyflie aufzubauen, braucht man einen sogenannten URI (Uniform Resource Identifier), die Verbindungsadresse:
...
Die Zeichenabfolge beinhaltet vier Variablen, die mit \bodyCode{<>} markiert sind. Diese Variablen sorgen für eine eindeutige Definition des Kommunikationsweges. Die \hTeLi{tab:uri}{folgende Tabelle} erklärt, wie die einzelnen Felder gewählt werden sollten. Dabei steht \inlinemath{m} für die Anzahl angeschlossener Crazyradio-Dongles.

\begin{table}[H]
    \centering
    \caption{Variablen eines URIs}
        \label{tab:uri}
    \begin{tabularx}{\textwidth}{l X X >{\hsize=0.37\hsize}X}
        \toprule
        \textbf{Variabel} & \textbf{Beschreibung} & \textbf{Zweck} & \textbf{Optionen} \\
        \midrule
        \bodyCode{<interface>} & Index des angeschlossenen Dongles & Zwischen mehreren Funkadaptern unterscheiden & \bodyCode{0} bis \inlinemath{m-1} \\
        \addlinespace[3pt]
